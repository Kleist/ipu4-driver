...
- `.github/workflows/ci.yml` — bootstrap + build + kunit on every PR and on push to `main`/`master`. A `pins` setup job reads `.github/kernel-pins.json` and emits a matrix-shaped output; `build-and-kunit` then fans across `fail-fast: false` so per-PR CI is deterministic. The 6.18 leg is expected red until the corresponding compat shims land in `kernel/ipu4/ipu4-compat.h`. The 7.0 leg builds clean on the existing shim cascade.
...
- `.github/workflows/bump-kernel-pins.yml` — Monday 05:30 UTC cron that resolves the latest stable point release for each kernel track, rewrites `.github/kernel-pins.json` (the single source of truth that `ci.yml`, `vm-smoke.yml`, and `vm-smoke-weekly.yml` all read at workflow start), and opens **one PR per track** via `peter-evans/create-pull-request` when a value changed. Matrix-fanned over `[6.12, 6.18, 7.0]` with `fail-fast: false`, so a single track's bump can land independently of stalled review on the others. The script that does the actual rewrite is `tools/bump-kernel-pins.sh` (accepts `--key <track>`). Note: bot-opened PRs don't fire CI under the default `GITHUB_TOKEN`; set a `BUMP_PAT` secret to lift that, or close-and-reopen the PR by hand.
...
- `.github/workflows/vm-smoke.yml` — full-VM boot + probe-smoke + streamon-smoke + mmiotrace + `compare-mmio` divergence report on every PR and on push to `main`/`master`. Pinned to the **6.12 leg only** (read from `.github/kernel-pins.json`, the same source `ci.yml` uses); 6.18 and 7.0 run weekly via `vm-smoke-weekly.yml`. A thin `workflow_call` caller — the body lives in `vm-smoke-reusable.yml`.
- `.github/workflows/vm-smoke-weekly.yml` — Sundays 07:00 UTC + `workflow_dispatch`. Matrix caller covering every non-6.12 track in `.github/kernel-pins.json` (today: 6.18 + 7.0).
