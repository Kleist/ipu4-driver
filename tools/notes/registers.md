...
| 0x64000-0x64fff | CSI2 port 0 | `ipu6-isys-csi2.c` per-port block | R/W latched per port. RX_ENABLE/CONFIG/DLY_CNT_* / per-trio EDGE/MASK/ENABLE/LEVEL_NOT_PULSE all latched; STATUS reads `status & enable`; CLEAR is W1C. Step 5b sets PART_IRQ_STATUS bit 0 (FS_VC0) on every PIN_DATA_READY delivery so the driver's CSI2 ISR runs and exercises the per-port read/W1C path. | inferred |
| 0x65000-0x65fff | CSI2 port 1 | `ipu6-isys-csi2.c` per-port block | Same handler logic as port 0 — one entry in the model's `csi2[]` array. Silicon's `data/trace.txt` doesn't exercise this port, so it stays at zero in the coverage report; landing it now means a future driver path that does enable port 1 is handled without a `LOG_UNIMP` fallback. | guess |
| 0x66000-0x66fff | CSI2 port 2 | `ipu6-isys-csi2.c` per-port block | Same as port 1. | guess |
| 0x67000-0x67fff | CSI2 port 3 | `ipu6-isys-csi2.c` per-port block | Same as port 1. | guess |
| 0x6c000-0x6c7ff | CSI2 port 4 | `ipu6-isys-csi2.c` per-port block | Same as port 1. Note: ports 4 and 5 share a 0x1000 BAR window split into 0x800 halves (`kernel/ipu4/ipu4-platform-isys-csi2-reg.h:13-15`); the model resolves that via `ipu4_csi2_resolve()`. | guess |
| 0x6c800-0x6cfff | CSI2 port 5 | `ipu6-isys-csi2.c` per-port block | Same as port 4. | guess |
