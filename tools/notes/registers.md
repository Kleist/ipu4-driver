...
| 0x108000-0x1080ff | ISYS | `ipu6-fw-com.c` DMEM syscom window   | Backed by a flat `uint32_t[0x40]` array. Step 2 added a syscom command parser: writes to `SYSCOM_CONFIG_REG` (DMEM[1], offset 0x004) latch the IPU IOVA of the syscom config struct and invalidate the cached queue descriptors; writes to `SYSCOM_STATE_REG` (DMEM[2], offset 0x008) with the `UNINIT` magic reset the per-stream send counters; writes to any msg-send `WR_REG` slot (DMEM[10..24] step 2, offsets 0x028..0x060 step 8) are detected as token bumps and trigger a STREAM_OPEN_DONE / STREAM_START_AND_CAPTURE_ACK response on count 1/2 respectively. `SEND_RD_POS` (offset 0x02c) echoes `SEND_WR_POS` on read; `SYSCOM_STATE` always reads `SYSCOM_STATE_READY` (`0x57A7E001`); `RECV_WR_POS` (offset 0x070) is bumped by the model when posting responses. | inferred |
