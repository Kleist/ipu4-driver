...
| 0x108000-0x1080ff | ISYS | `ipu6-fw-com.c` DMEM syscom window   | Backed by a flat `uint32_t[0x40]` array: every slot is a latch with two exceptions — `SEND_RD_POS` (offset 0x2c) echoes `SEND_WR_POS` so the driver sees "firmware has caught up" and doesn't block on the ring filling, and `SYSCOM_STATE` (offset 0x08) always reads as `SYSCOM_STATE_READY` (`0x57A7E001`) so `ipu6_fw_com_ready()` terminates on the first poll. `RECV_WR_POS` (offset 0x70) stays at whatever the driver last wrote (0 after reset); silicon sees real firmware updates, so compare.py flags it as value_mismatch until firmware simulation lands. | inferred |
