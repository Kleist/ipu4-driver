...
static inline u32 ipu_debug_readl(const volatile void __iomem *addr, const char*, unsigned, const char* func) {
...
static inline void ipu_debug_writel(u32 value, volatile void __iomem *addr, const char*, unsigned, const char* func) {
       if (func != ipu_last_rw_func) {
               mmiotrace_printk("next mmio from %s\n", func);
               ipu_last_rw_func = func;
       }
...
#if defined(IPU_LOG_MARK_RW_LOCATIONS)
// Replace readl/writel macros with ones that debugs, the original readl/writel is used above for the actual read/write
#undef readl
#define readl(ADDR) ipu_debug_readl(ADDR, __FILE__, __LINE__, __func__)

#undef writel
#define writel(VALUE, ADDR) ipu_debug_writel(VALUE, ADDR, __FILE__, __LINE__, __func__)
#endif
